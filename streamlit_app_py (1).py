...
def recommend_top_foods_by_cluster(df, condition, healthy_cluster_label=0, top_n=10):
    ...
    top_foods = df_filtered.sort_values(by='score', ascending=False).head(top_n)[['food_name', 'score']]
    return top_foods

# UI
st.set_page_config(page_title="Health-Based Food Recommender", layout="wide")
st.title("🥗 Smart Health-Based Food Recommendation System")

age = st.slider("Select Age", 10, 90, 25)
gender = st.radio("Select Gender", ["Male", "Female"])
condition = st.selectbox("Choose Health Condition", ["Diabetes", "Obesity", "High_BP", "Low_BP"])

st.subheader(f"🔍 Top 10 Foods Recommended for {condition}")
top_foods = recommend_top_foods_by_cluster(df, condition.lower())
st.dataframe(top_foods, use_container_width=True)

st.subheader("🧬 Health Cluster Distribution")
label_col = label_map[condition.lower()]
st.bar_chart(df[label_col].value_counts())

st.subheader("📊 3D PCA Clustering Visualization")
selected_features = list(condition_weights[condition.lower()].keys())
df_clean = df[selected_features].fillna(0)
pca = PCA(n_components=3)
df_pca = pd.DataFrame(pca.fit_transform(df_clean), columns=["PC1", "PC2", "PC3"])
cluster_labels = df[label_col].values

fig = plt.figure(figsize=(8, 6))
ax = fig.add_subplot(111, projection='3d')
ax.scatter(df_pca["PC1"], df_pca["PC2"], df_pca["PC3"], c=cluster_labels, cmap='viridis', s=40)
ax.set_xlabel("PC1")
ax.set_ylabel("PC2")
ax.set_zlabel("PC3")
st.pyplot(fig)
...
st.subheader("📋 Full Dataset (Explore Nutrients)")
with st.expander("Click to Expand Table"):
    st.dataframe(df[['food_name', 'energy_kcal', 'carb_g', 'protein_g', 'fat_g', 'fibre_g']])

st.subheader("📉 Macronutrient Comparison of Recommended Foods")
nutrient_cols = ['carb_g', 'protein_g', 'fat_g']
compare_df = df[df['food_name'].isin(top_foods['food_name'])][['food_name'] + nutrient_cols].set_index('food_name')
...
df = pd.read_csv("Labeled_Data.csv")  # Replace with path if needed

# Map condition to health label column
label_map = {
    'diabetes': 'Health_Label_Diabetes',
    'obesity': 'Health_Label_Obesity',
    'high_bp': 'Health_Label_HighBP',
...
# 3D PCA Plot
st.subheader("📊 3D PCA Clustering Visualization")
# Use top condition weights for PCA
selected_features = list(condition_weights[condition.lower()].keys())
df_clean = df[selected_features].fillna(0)
pca = PCA(n_components=3)
df_pca = pd.DataFrame(pca.fit_transform(df_clean), columns=["PC1", "PC2", "PC3"])
...
fig = plt.figure(figsize=(8, 6))
ax = fig.add_subplot(111, projection='3d')
scatter = ax.scatter(df_pca["PC1"], df_pca["PC2"], df_pca["PC3"], c=cluster_labels, cmap='viridis', s=40)
ax.set_xlabel("PC1")
ax.set_ylabel("PC2")
ax.set_zlabel("PC3")
st.pyplot(fig)
...
try:
    ...
    st.warning("Silhouette score could not be calculated. Check clusters.")
...
st.subheader("📉 Macronutrient Comparison of Recommended Foods")
nutrient_cols = ['carb_g', 'protein_g', 'fat_g']
compare_df = df[df['food_name'].isin(top_foods['food_name'])][['food_name'] + nutrient_cols]
compare_df = compare_df.set_index('food_name')
st.bar_chart(compare_df)
